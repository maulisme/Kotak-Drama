...
async function fetchCategories() {
  try {
    const response = await fetch('https://www.dramaboxdb.com/_next/data/dramaboxdb_prod_20250918/in/genres.json')
    ...
    return {
      ...
      dramas: data.pageProps.bookList.map((item: any): Drama => ({
        ...
        jsonUrl: `https://www.dramaboxdb.com/_next/data/dramaboxdb_prod_20250918/in/movie/${item.originalBookId}/${item.bookNameLower}.json`
      }))
    }
  } catch (error) {
...
async function fetchCategoryDramas(categoryId: number) {
  try {
    const response = await fetch(`https://www.dramaboxdb.com/_next/data/dramaboxdb_prod_20250918/in/genres/${categoryId}.json?typeTwoId=${categoryId}`)
    const data = await response.json()
...
async function fetchCategoryContent(slug: string) {
  try {
    ...
    const categoriesResponse = await fetch('https://www.dramaboxdb.com/_next/data/dramaboxdb_prod_20250918/in/genres.json')
    ...
    const response = await fetch(`https://www.dramaboxdb.com/_next/data/dramaboxdb_prod_20250918/in/genres/${category.id}.json?typeTwoId=${category.id}`)
    const data = await response.json()
